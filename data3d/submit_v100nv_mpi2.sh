...
#SBATCH -J benchmark
#SBATCH -N 1 -n 2 -A fuac4_feltor --exclusive --gres=gpu:2 -p m100_fua_prod
#SBATCH --time=12:00:00

###please specify a info file (for humans)
#SBATCH -o "benchmark_v100nv_mpi2.info"
FILENAME='benchmark_v100nv_mpi2' # name of output file
...
mpirun -gpu -n 2 ./ping_mpit

COMMAND='mpirun -gpu -n 2 ./cluster_mpib'
FILE=""$FILENAME".csv"
    ...
    for o in 3 4; do
        for N in 34 68 136 272; do
            for Nz in 16 32 64 128; do
                echo "1 1 2 $o $N $N $Nz" | $COMMAND ./cluster_mpib >> $FILE
            done
        done
    done
done
